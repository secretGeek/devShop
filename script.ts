...
function drawXP(xp: string | number) {
...
function drawStory(key: string, stories: { [x: string]: any; }, el: HTMLElement, top: boolean) {
  var s = el.querySelector('#' + key);
  var avatar = "";
  var busy = "";
...
function getSkillsDiv(skills: any) {
  var result = "";
  for(var s of skills) {
    var s1 = "";
    switch(s) {
...
//In any action there is a receiver and a doer. The doer can be 'the player' or a dev, tester, or BA.
  ...
  */
  ...
  document.onkeypress = function(e) {
    switch (e.key) {
      case "1":
        var firstStory = Object.keys(stories).map(k => stories[k]).filter(s => s.column === 'ba' && s.status === 'lead' && !s.person)[0];
        if (firstStory)
          clickReceiver('r' + firstStory.id);
        break;
      case "2":
        var firstStory = Object.keys(stories).map(k => stories[k]).filter(s => s.column === 'dev' && s.status === 'story' && !s.person)[0];
        if (firstStory)
          clickReceiver('r' + firstStory.id);
        break;
      case "3":
        break;
      case "4":
        var firstStory = Object.keys(stories).map(k => stories[k]).filter(s => s.column === 'test' && s.status === 'story' && !s.person)[0];
        if (firstStory)
          clickReceiver('r' + firstStory.id);
        break;
      case "5":
...
function clickDoer(id: any) {
  ...
  for(const skill of people[game.SelectedDoer].skills) {
...
function tryDo(doId: string, receiveId: string) {
...
function doIt(doId: string, receiveId: string) {
...
function getTaskVerb(column: any) {
  switch(column) {
...
function getTaskFactor(column: any){
  switch(column) {
    case "ba": return 0.3;
    case "dev": return 0.3;
