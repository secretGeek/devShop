...
function getNewHireHtml(item: HireItem) {
  ...
  }' id='storeitem-${item.id}'><div class='button' id='hire-button-${item.id}'>${formatPrice(
    item.price
  )}</div><span class='hireIcon'>${
    item.icon
  }</span> <span id='describe-store-item-${item.id}' class='describe' title='more information'>❓</span><span class='item-name'>${
    item.name
  }</span></div>`;
}
...
function getStoreItemHtml(item: StoreItem) {
  return `<div class='storeItem-catalog ${
    item.enabled ? "item-enabled" : "item-disabled"
  }' id='storeitem-${item.id}'><div class='button' id='store-button-${item.id}'>${formatPrice(
    item.price
